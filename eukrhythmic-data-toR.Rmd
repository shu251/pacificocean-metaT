...
tic()
tpm_all <- fread("../../../scratch/user/skhu/SPOT-ALOHA/03-abundance_tables/TPMTable_ByContig.csv", 
                 verbose = TRUE,
                 drop = 1,
                 showProgress = TRUE,
                 nThread = 32)
toc() # 1664.89 sec elapsed and at 22.39 GB in RStudio
str(tpm_all)
# colnames(tpm)
...
```{r}
# Should all equal 1E6
sum(tpm_all$SRR11178168)
sum(tpm_all$SRR6048898)
```


## Totals, stats, annotations

```{r}
head(annot_df)
length(unique(annot_df$ShortSeqID))
length(unique(tpm_all$ShortSeqID))
```


Isolate eukaryote only Seq IDs
```{r}
...
# class(shard_sample)
euk_only <- shard_sample %>% 
  filter(Domain == "Eukaryota") %>% 
  collect()

colnames(euk_only)
euk_seqid <- as.character(unique(euk_only$ShortSeqID))
```
...
tic()
tpm_all_euk <- tpm_all %>% 
  filter(ShortSeqID %in% euk_seqid) 
toc()
...
tic()
tpm_all_euk_clr <- compositions::clr((t(tpm_all_euk) %>% 
  column_to_rownames(var = "")))
toc()
# head(tpm_all_euk_clr)

tic()
tpm_all_euk_long <- tpm_all_euk %>% 
  pivot_longer(cols = starts_with("SRR"))
toc()

# Save data table outputs
save(tpm_all_euk, tpm_all_euk_clr, tpm_all_euk_long, file = "../../../scratch/user/skhu/SPOT-ALOHA/tpm_all_datafiles.RData")
```
...
<<<<<<< HEAD
tax_df <- tax_key %>% collect() #Use collect to actually SAVE sql db steps as a dataframe.
class(tax_df)
...
# class(metadata_df)
tic()
tpm_ALL_mean_reps_annot <- tpm_all_euk_long %>% 
  filter(value > 0) %>% 
  left_join(metadata_df, by = c("name" = "RUN")) %>% 
...
```{r}
save(tpm_ALL_mean_reps_annot, file = "/scratch/user/skhu/SPOT-ALOHA/Avg_tpm_ALL_acrossreps_annot.RData")
```

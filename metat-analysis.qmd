...
```
make a toy dataset to use here and write a script later
```{r}
...
ggplot(counts_zscore, aes(x = SequenceID, y = SAMPLENAME, fill = z_score_transcript)) +
...
Making toy data set first, to test code.
Need to input the processed center normalized data for below. 

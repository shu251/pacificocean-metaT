...
*optional*: subset data to keep only transcripts with >10 tpm.
...
Use this for the majority of downstream analyses.
...
load("/scratch/group/hu-lab/pacocean-metaT/Avg_scaled_tpm_08222023.RData")
```
...
```{r}
save(df_ctr_norm, file = "/scratch/group/hu-lab/pacocean-metaT/normed_center_df_08222023.RData")
```
...
First to do some data quality control
```{r}
head(counts_withreps)
glimpse(counts_withreps)
```
...
dim(qc_df_wreps)[1] / dim(counts_withreps)[1]
# head(qc_df_wreps)

qc_mean_counts <- mean_counts_df %>%
  filter(rowSums(across(where(is.numeric), ~.x!=0))>1) %>% # samples appear more than 2 times
  filter(rowSums(across(where(is.numeric)))>=5)

dim(qc_mean_counts)[1] / dim(mean_counts_df)[1]
```



```{r}
scale_z_wreps <- scale(as.matrix(qc_df_wreps), center = TRUE)
range(scale_z_wreps); median(scale_z_wreps)

scale_z_means <- scale(as.matrix(qc_mean_counts), center = TRUE)
range(scale_z_means); median(scale_z_means)
```

```{r}
save(scale_z_means, scale_z_wreps, file = "/scratch/group/hu-lab/pacocean-metaT/zscore_outputs.RData")
```
...
```{r}
glimpse(scale_z_means)
# glimpse(taxfxn)
df_zscale_means <- as.data.frame(scale_z_means) %>%
...
df_zscale_means %>%
  ...
  ggplot(aes(x = ANNOTATION, y = SAMPLENAME, fill = zscore_TPM)) +
    geom_tile() + 
...
sample_order_month_MEAN <- c("mean.July_5m", "mean.July_DCM", "mean.July_150m", "mean.July_1000m", "mean.March_5m", "mean.March_DCM", "mean.March_150m", "mean.March_1000m", "mean.Catalina", "mean.PortofLA", "mean.SPOT_surface", "mean.SPOT_150m", "mean.SPOT_890m")

```


```{r}
mean_counts_df_reformat <- mean_counts_df %>% 
  sample_n(50) %>% 
  filter(rowSums(across(where(is.numeric)))>=5) %>% # sum of transcript has to be >=5
  rownames_to_column(var = "SequenceID") %>% 
  # left_join(taxfxn) %>%
  pivot_longer(cols = starts_with("mean."), names_to ="SAMPLE", values_to = "TPM_whole") %>% 
  # mutate(ANNOTATION = case_when(
  #   KEGG_ko == "-" ~ PFAMs,
  #   TRUE ~ KEGG_ko
  # )) %>%
  # filter(!is.na(ANNOTATION)) %>% 
  # filter(ANNOTATION != "-") %>% 
  mutate(SAMPLE_NAME = factor(SAMPLE, levels = sample_order_month_MEAN, labels = sample_order_month_RENAME)) %>% 
  group_by(SequenceID, SAMPLE_NAME) %>%
    summarize(TPM = mean(TPM_whole))

# dataset mean and sd
mean_dataset <- mean(mean_counts_df_wannot$TPM)
sd_dataset <- sd(mean_counts_df_wannot$TPM)

zscore_trans_df <- mean_counts_df_wannot %>% 
  group_by(SequenceID, SAMPLE_NAME) %>%
    summarise(SUMTPM = sum(TPM),
              zscore = ((SUMTPM - mean_dataset) / sd_dataset))
```
```{r}
...
```{r}
glimpse(zscore_trans_df)

```


Core RObjects from above are stored: /scratch/group/hu-lab/pacocean-metaT

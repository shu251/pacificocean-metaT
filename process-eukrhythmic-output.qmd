...
Description: tximport imports transcript-level estimates from Salmon and summarizes abundances, counts, and transcript lengths to output transcript-level matrices (see txOut argument).
...
Save checkpoint step on HPC

```{r}
save(txi_makecounts, file = "/vortexfs1/scratch/sarahhu/txi_makecounts.RData")
...
_This is the step that I cannot run on poseidon_
